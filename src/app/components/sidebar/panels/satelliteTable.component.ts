...
import { Component, ViewChild, ViewContainerRef, ComponentFactoryResolver, ComponentRef, Inject } from '@angular/core';
...
export class SatelliteTableComponent {
    ...
    private clearAllChildren(): void {
        this.childComponentRefs.forEach(ref => {
            const vcrIndex: number = this.planetNamesRef.indexOf(ref as any); 
            this.planetNamesRef.remove(vcrIndex);
            ref.destroy();
        });
